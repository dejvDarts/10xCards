...
- **Auth, per test (no shared `storageState`):** every test still creates its own disposable user via `createTestUser()` — there is no single long-lived shared session, on purpose, since tests mutate per-user DB rows and run `fullyParallel`. Two ways to get a signed-in `page`:
  - **Default — inject the session, skip the UI**: `signInViaCookie(context, user, baseURL)` (`tests/e2e/helpers/auth.ts`) signs in through the same `@supabase/ssr` flow the integration tests use and adds the resulting cookies straight to the browser context. Use this whenever authentication is a precondition, not the risk under test (e.g. `full-journey.spec.ts`).
  - **When the risk IS the login/session flow itself**: drive the real `/auth/signin` form (`getByRole("textbox", {name:"Email"})` → `getByLabel("Password", {exact:true})` → `getByRole("button", {name:"Sign in"})`) — see `seed.spec.ts`. Don't swap this one for cookie injection; the browser round trip (`Set-Cookie` landing, surviving navigation, sign-out re-engaging the gate) is the thing being tested.
