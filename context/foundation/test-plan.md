...
| 5   | Full-journey e2e coverage      | Prove the PRD's primary success flow survives the seam between phases: generate → accept → study session | #7            | e2e (+ integration for the backend seam) | complete | `context/changes/full-journey-e2e-coverage/`                   |
...
- Two rollout phases have shipped e2e so far: Phase 1 (Risk #4, page-gating) and Phase 5 (Risk #7, full generate → accept → study journey — `context/changes/full-journey-e2e-coverage/`). E2E stays "reconsider only if a risk needs the full deployed shape (browser + cookie + handler crossing)" — it is not a default layer.
