...
| 7   | The full "generate → review/accept → study session" user journey breaks at the seam between phases, even though each phase's own tests pass (e.g. an accepted flashcard never becomes visible to the due list, or state doesn't survive a real page reload/navigation between the AI workshop and the dashboard) | High   | Medium     | PRD Success Criteria (Primary): "Pełny przepływ end-to-end działa: użytkownik loguje się, wkleja tekst, otrzymuje propozycje fiszek generowane przez AI, przegląda je (akceptuje / edytuje / odrzuca), zaakceptowane fiszki zapisują się na koncie, a następnie uczy się w sesji powtórek..."; Phase 2 rollout note (§6.6): "Generated rows are status:\"pending\" ... invisible to getDueFlashcards/recordReview ... until promoted — cross-flow tests must bridge that seam explicitly. No such cross-flow test is in this phase." |
...
**Risk #7 added 2026-09-13.** Sourced from PRD Success Criteria (Primary) —
the full generate → review/accept → study-session flow is literally the
product's headline success metric — and from the Phase 2 rollout note
(§6.6) that already flagged the `pending` → `accepted` promotion seam as
untested cross-flow territory. Unlike Risks #1–#6, this is the first risk
in the map that plausibly justifies e2e over unit/integration: it is a
multi-step, browser-driven journey crossing auth → routing → API → DB, not
a single endpoint contract or pure function (see §4, §6.3).
...
| #7   | A user can generate proposals from pasted text, accept at least one, and then see that same card appear in — and be ratable from — a real study/review session, with state surviving an actual page navigation/reload (not just sequential API calls)                              | "each phase's tests are green" does not mean the seam between them holds — a generate-route test and a due-list test each assume the other side produced a compatible row; only a flow that actually promotes generate → accept → due proves the boundary. Also challenge whether e2e is even the right layer: the backend promotion (status transition + due-list visibility) can be proven more cheaply by chaining real handler calls in an integration test — e2e should earn its cost only for the browser-driven, cross-page-navigation portion the PRD's success criterion describes | which status transition marks a card "study-ready" (`pending` → `accepted`, via the same `PATCH` route hardened in Phase 3), whether `getDueFlashcards` filters strictly on `status = 'accepted'`, whether a freshly-accepted card's `due` is already `<= now` | e2e for the browser-driven journey (generate page → accept a proposal → navigate to the study page → rate the card); try a cheaper chained-handler integration test (`generate` → `PATCH accept` → `getDueFlashcards`/`recordReview`) first to pin the backend seam alone | asserting only that each phase's isolated test is green; treating a chain of direct API calls as proof the real user-driven browser journey works, when the risk is specifically about the browser/navigation seam |
...
| 4   | Quality-gates wiring           | Add the test suite to CI alongside the existing lint + build gates                                       | cross-cutting | gates                                    | complete | `context/archive/2026-09-10-quality-gates-wiring/`             |
| 5   | Full-journey e2e coverage      | Prove the PRD's primary success flow survives the seam between phases: generate → accept → study session | #7            | e2e (+ integration for the backend seam) | complete | `context/archive/2026-09-13-full-journey-e2e-coverage/`        |
...
| e2e                  | Playwright             | 1.63.0  | `tests/e2e/seed.spec.ts` covers Risk #4's page-gating half (added outside the phased rollout, per §6.3); Risk #7 (added 2026-09-13) is the next case the seed pattern should extend to — checked: 2026-09-13                       |
...
- **Playwright** is installed (`playwright.config.ts`, `tests/e2e/`, `npx playwright test`). `tests/e2e/seed.spec.ts` is the reference pattern (`getByRole` locators, state-based waits, unique test-data identifiers via `createTestUser()`, cleanup in `afterEach`) — added for Risk #4's page-gating half (the one thing a middleware unit test can't see: the real browser round trip on `Set-Cookie`).
- Two rollout phases have shipped e2e so far: Phase 1 (Risk #4, page-gating) and Phase 5 (Risk #7, full generate → accept → study journey — `context/archive/2026-09-13-full-journey-e2e-coverage/`). E2E stays "reconsider only if a risk needs the full deployed shape (browser + cookie + handler crossing)" — it is not a default layer.
- **Auth without the UI, per test:** `tests/e2e/helpers/auth.ts`'s `signInViaCookie(context, user, baseURL)` signs in via the same `@supabase/ssr` flow the integration tests use (`cookieHeaderFor`) and injects the resulting session cookies straight into the browser context — no `/auth/signin` round trip. Each test still creates its own disposable user (`createTestUser()`); this only skips the login _form_, preserving per-test isolation without a shared `storageState`. Use it whenever auth is a precondition, not the risk under test (`full-journey.spec.ts`). A test whose risk **is** the login/session flow (`seed.spec.ts`, Risk #4) must keep driving the real form — that round trip is what it protects.
- **Known boundary limitation:** the app's dev server (`playwright.config.ts`'s `webServer`) is a separate process, so `page.route()` only intercepts requests made **from the browser** — an API route's own server-side outbound call (e.g. the AI provider fetch inside `generate.ts`) is invisible to it. Mocking that boundary from e2e would need dedicated test infrastructure (a configurable stub endpoint) that doesn't exist yet; don't force an e2e test past this limit — fall back to a cheaper layer or flag the infra gap instead (see the Risk #3 discussion this project already had).
...
- Strategy (§1–§5) last reviewed: 2026-09-13 (added Risk #7 — full generate → accept → study-session journey)
- Stack versions last verified: 2026-09-13 (Playwright 1.63.0 entry corrected — it was already installed)
