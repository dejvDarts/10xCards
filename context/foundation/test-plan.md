...
| 4 | Auth/session gating regresses on a protected route or API endpoint | High | Medium | hot-spot dirs `src/components/auth` (6 commits/30d), `src/pages/auth` (5 commits/30d); AGENTS.md protected-route rule |
...
| #4 | An unauthenticated request to any protected route/endpoint is redirected/rejected; an authenticated user is never blocked from their own pages | "the page renders without an error" does not mean the auth check ran | which routes are protected, what middleware does with an absent/expired session, redirect target | integration (no/expired session vs. valid session per route) | testing one protected route and assuming it generalizes; asserting status code without checking redirect destination |
...
|---|---|---|---|---|---|---|
| 1 | Critical-path coverage | Bootstrap the test runner and defend the two access-control risks first | #1, #4 | unit + integration | change opened | `context/changes/testing-critical-path-coverage/` |
