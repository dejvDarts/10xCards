...
| 5   | Full-journey e2e coverage      | Prove the PRD's primary success flow survives the seam between phases: generate → accept → study session | #7            | e2e (+ integration for the backend seam) | change opened | `context/changes/full-journey-e2e-coverage/`                   |
...
- Still not a rollout phase — e2e stays "reconsider only if a risk needs the full deployed shape (browser + cookie + handler crossing)," now true for Risk #7 (§2) in addition to Risk #4.
